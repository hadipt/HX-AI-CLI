# Provide context with AURORA.md files

Context files, which use the default name `AURORA.md`, are a powerful feature
...
1.  **Global context file:**
    - **Location:** `~/.gemini/AURORA.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Project root and ancestor context files:**
    - **Location:** The CLI searches for a `AURORA.md` file in your current
...
3.  **Sub-directory context files:**
    - **Location:** The CLI also scans for `AURORA.md` files in subdirectories
...
### Example `AURORA.md` file

Here is an example of what you can include in a `AURORA.md` file at the root of
...
- **`/memory refresh`**: Forces a re-scan and reload of all `AURORA.md` files
  from all configured locations.
- **`/memory add <text>`**: Appends your text to your global
  `~/.gemini/AURORA.md` file. This lets you add persistent memories on the fly.

## Modularize context with imports

You can break down large `AURORA.md` files into smaller, more manageable
...
**Example `AURORA.md` with imports:**
...
While `AURORA.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "AURORA.md"]
  }
}
```
